...
const twilioPhoneNo = process.env.TWILIO_PHONE_NUMBER; //to grab data from .env file which we want to keep private
...
app.get("/api/compliments", async (req, res) => {
  ...
  const sentMessages = await client.messages.list({ from: twilioPhoneNo });
